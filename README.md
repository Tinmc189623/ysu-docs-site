...
**正文补了 frontmatter。** 以前标题和摘要由 Nuxt Content 从首个 H1 和首段自动抽，正文里那个 H1 是渲染出来的。Fumadocs 把标题当元数据，正文里再留一个 H1 就重复了，所以迁移时把标题搬进了 frontmatter、从正文里删掉，顺带把首段抽成了 `description`。四十九篇都过了一遍。
...
顺序、分组显示名、分组说明都写在 `meta.json` 里。根上那份 `content/docs/meta.json` 排六个分组的先后，各分组目录里那份排组内每一篇的先后，同时提供侧边栏上的分组名和首屏卡片上那句说明。
...
搜索在 Vercel 上是一条 Serverless Function，索引在冷启动时建。实测四十九篇的量，第一次请求（含建索引）185 毫秒，之后每次十几毫秒，不用管。
...
- `/llms-full.txt` 把四十九篇正文拼成一个纯文本文件
...
content/docs/                   正文，49 篇 markdown，分六个目录，每目录一份 meta.json
