---
title: "已知限制"
description: "支持范围是按特性列的表。这一页换个角度：从你会看到什么现象出发。"
---
...
选择器里 `:hover`、`:active`、`:focus`、`:visited` 的匹配逻辑是写好了的，但匹配需要一个 `ElementState` 参数，而没有任何地方提供它——四项全为假。

所以悬停效果、焦点样式、已访问链接的变色都不会出现。命中测试本身是正常的，`:link` 这类不依赖状态的伪类照常工作。
...
这是**对的行为**，不是 bug。内核按给它的视口宽度排版，宽度变了就重排。所以宽度收窄时换行位置会变、文字占的行数会变——而不是整体像图片一样被拉大。
